...
#--------FUNCTIONS--------------------------------------------
def agaon(): #<--FUNTION HEADER
    '''this funtion askes a user if they'd like to enter another temp, checks the responce for validity, and then returns '''
    ...
    while ans != "y" amd ans != "n"
        print("*****INVALID ENTRY!******")
        ans = input("/t/tWould you like to enter another temperature? [y/n]").lower()
        return ans

def converter(f)
    '''this funtion is passed a tempF value('f' parameter), converts to celsius, and returns the tempC value'''
...
while answer == "y" or answer == "Y" and temp_total >=0: 
    ...
    print("f\n\t\tTEMP# {temp_count}\tTEMP {1:.1f}F = TEMP {2:.1f}C\n")
    ...
    answer = again()
    #the return value of the funtion of this funtion will replace this call as final result for "answer"
